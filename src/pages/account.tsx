...
function AccountPage() {
  const { user, requestEmailChange, externalAuths, fetchCurrentUser, unlinkExternalAuth } = useAuth();
  const { pb } = useClient();
  ...
  return (
    ...
    >
      ...
      {
        externalAuths.length > 0 ? externalAuths.map((auth) => (
          <div className="group relative flex items-center justify-end" key={auth.provider}>
            <OAuthButton provider={auth.provider} onClick={() => {}} disabled={true} key={auth.provider} />
              <X 
